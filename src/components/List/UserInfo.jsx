...
const UserInfo = ({ user, isMobile }) => {
  ...
  return (
    <>
      {user && (
        <div
          className={`${isMobile ? "bg-black" : "bg-white/5 backdrop-blur-lg"} p-4 md:p-6 flex gap-3 items-center justify-between  border-b border-white/10 `}
        >
          <div className="flex items-center gap-3 md:gap-4 min-w-0 flex-1 ">
            <Avatar className="w-9 h-9 md:w-11 md:h-11 flex-shrink-0">
              <AvatarImage src="/john-bigman-headshot.png" alt="John Bigman" className="object-cover" />
            ...
            <div>
              ...
              <h3 className="text-white/70 text-sm md:text-md truncate ">@{user?.username}</h3>
            </div>
          </div>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="bg-white/5 backdrop-blur-lg border border-white/10">
                ...
                <DropdownMenuGroup>
                  ...
                  <DropdownMenuItem className="hover:bg-white/10 text-white/70" onClick={userLogOut}>
                    Logout
                  </DropdownMenuItem>
                </DropdownMenuGroup>
              </DropdownMenuContent>
